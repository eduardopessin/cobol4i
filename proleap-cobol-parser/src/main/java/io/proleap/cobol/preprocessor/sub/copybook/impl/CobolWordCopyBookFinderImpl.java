...
public class CobolWordCopyBookFinderImpl implements CobolWordCopyBookFinder {
	...
	public File findCopyBook(final CobolParserParams params, final CobolWordContext ctx) {
		return findCopyBook(params, ctx, null);
	}

	public File findCopyBook(final CobolParserParams params, final CobolWordContext ctx, final String libraryName) {
		// If a library name is specified, try library mappings first
		if (libraryName != null && params.getCopyBookLibraryMappings() != null) {
			final File libraryDir = params.getCopyBookLibraryMappings().get(libraryName.toUpperCase());
			if (libraryDir != null && libraryDir.isDirectory()) {
				final File found = findCopyBookInDirectory(libraryDir, params, ctx);
				if (found != null) {
					return found;
				}
			}
