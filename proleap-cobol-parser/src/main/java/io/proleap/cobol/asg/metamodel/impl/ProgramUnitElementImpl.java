...
public class ProgramUnitElementImpl extends CompilationUnitElementImpl implements ProgramUnitElement {
	...
	protected Call createCall(final QualifiedDataNameFormat1Context ctx) {
		...
		if (isQualifiedData) {
			...
			// Do NOT reverse: COBOL "X OF Y OF Z" has Y=nearest ancestor, Z=furthest
			// isSameInData walks bottom-up, so nearest-first order is correct
	...
	protected Call createCall(final TableCallContext ctx) {
		...
		if (result == null) {
			// Resolve the qualified data name using the same logic as QualifiedDataNameFormat1
			// (determineName(TableCallContext) returns full text like "FIELDOFGroup" which fails lookup)
			final Call resolvedCall = createCall(ctx.qualifiedDataName());
			DataDescriptionEntry dataDescriptionEntry = null;

			if (resolvedCall != null) {
				final Call unwrapped = resolvedCall.unwrap();
				if (unwrapped instanceof DataDescriptionEntryCall) {
					dataDescriptionEntry = ((DataDescriptionEntryCall) unwrapped).getDataDescriptionEntry();
				}
			}

			if (dataDescriptionEntry == null) {
				// Fallback: try simple name lookup (for unqualified table calls)
				final String name = determineName(ctx);
				final List<DataDescriptionEntry> dataDescriptionEntries = findDataDescriptionEntries(name);
				if (!dataDescriptionEntries.isEmpty()) {
					dataDescriptionEntry = dataDescriptionEntries.get(0);
				}
			}
	...
	protected String getSymbol(final String name) {
		...
		// Normalize underscores to hyphens for IBM ILE COBOL compatibility.
		// AS/400 DDS COPY converts underscores to hyphens in generated copybooks,
		// but source programs may reference fields using either form.
		// Both DATINI_ANT and DATINI-ANT must resolve to the same symbol.
	...
	private boolean isSameInData(final DataDescriptionEntry candidateDataDescriptionEntry,
		...
		for (final QualifiedInDataContext parentCtx : parentInDataCtxs) {
			...
			// Walk up the ancestor chain to find a matching ancestor (not just direct parent)
