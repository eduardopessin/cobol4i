...
import io.proleap.cobol.asg.metamodel.procedure.commit.impl.CommitStatementImpl;
...
import io.proleap.cobol.asg.metamodel.procedure.rollback.impl.RollbackStatementImpl;
...
public class ScopeImpl extends CobolDivisionElementImpl implements Scope {
	...
	@Override
	public CommitStatement addCommitStatement(final CommitStatementContext ctx) {
		CommitStatement result = (CommitStatement) getASGElement(ctx);

		if (result == null) {
			result = new CommitStatementImpl(programUnit, this, ctx);
	...
	public DisplayStatement addDisplayStatement(final DisplayStatementContext ctx) {
		...
		if (result == null) {
			...
			if (ctx.displayWith() != null) {
				for (final DisplayWithContext withCtx : ctx.displayWith()) {
					result.addWith(withCtx);
				}
			}
	...
	public ExecSqlStatement addExecSqlStatement(final ExecSqlStatementContext ctx) {
		...
		if (result == null) {
			...
			// Filter out COBOL comment lines from EXEC SQL blocks before extracting text.
			// Comment lines have '*' in column 7, which the preprocessor converts to '*>' markers.
			// After removing the EXECSQL tag, comment lines start with "*>".
			final java.util.List<org.antlr.v4.runtime.tree.TerminalNode> allExecSqlTokens = new java.util.ArrayList<>();
			allExecSqlTokens.addAll(ctx.EXECSQLLINE());
			if (ctx.EXECSQLEND() != null) {
				allExecSqlTokens.add(ctx.EXECSQLEND());
			}
			// Sort by token index to preserve original order
			allExecSqlTokens.sort((a, b) -> Integer.compare(a.getSymbol().getTokenIndex(), b.getSymbol().getTokenIndex()));
			final java.util.List<org.antlr.v4.runtime.tree.TerminalNode> sqlLines = new java.util.ArrayList<>();
			for (final org.antlr.v4.runtime.tree.TerminalNode node : allExecSqlTokens) {
				final String nodeText = node.getText();
				// Remove EXECSQL tag to check if remaining text is a comment
				String untagged = nodeText.replace(CobolPreprocessor.EXEC_SQL_TAG, "")
						.replace(CobolPreprocessor.EXEC_END_TAG, "").trim();
				// Skip comment lines: those starting with *> or * after tag removal
				if (untagged.startsWith("*>") || untagged.startsWith("*")) {
					continue;
				}
				sqlLines.add(node);
			}
			String execSqlText = TagUtils.getUntaggedText(sqlLines, CobolPreprocessor.EXEC_SQL_TAG,
					CobolPreprocessor.EXEC_END_TAG);
	...
	public ReadStatement addReadStatement(final ReadStatementContext ctx) {
		...
		if (result == null) {
			...
			// IBM ILE: subfile
			if (ctx.SUBFILE() != null) {
				result.setSubfile(true);
			}

			// IBM ILE: format phrase
			if (ctx.readFormatPhrase() != null) {
				final ValueStmt formatValueStmt = createValueStmt(ctx.readFormatPhrase().identifier(),
						ctx.readFormatPhrase().literal());
				result.setFormatPhrase(formatValueStmt);
			}

			// IBM ILE: indicators phrase
			if (ctx.readIndicatorsPhrase() != null) {
				final Call indicatorsCall = createCall(ctx.readIndicatorsPhrase().identifier());
				result.setIndicatorsPhrase(indicatorsCall);
			}

			// IBM ILE: no data phrase
			if (ctx.readNoDataPhrase() != null) {
	...
	public RewriteStatement addRewriteStatement(final RewriteStatementContext ctx) {
		...
		if (result == null) {
			...
			// IBM ILE: format phrase
			if (ctx.rewriteFormatPhrase() != null) {
				final ValueStmt formatValueStmt = createValueStmt(ctx.rewriteFormatPhrase().identifier(),
						ctx.rewriteFormatPhrase().literal());
				result.setFormatPhrase(formatValueStmt);
			}

			// IBM ILE: indicators phrase
			if (ctx.rewriteIndicatorsPhrase() != null) {
				final Call indicatorsCall = createCall(ctx.rewriteIndicatorsPhrase().identifier());
				result.setIndicatorsPhrase(indicatorsCall);
			}
	...
	@Override
	public RollbackStatement addRollbackStatement(final RollbackStatementContext ctx) {
		RollbackStatement result = (RollbackStatement) getASGElement(ctx);

		if (result == null) {
			result = new RollbackStatementImpl(programUnit, this, ctx);
	...
	public WriteStatement addWriteStatement(final WriteStatementContext ctx) {
		...
		if (result == null) {
			...
			// IBM ILE: format phrase (take first if duplicated)
			if (ctx.writeFormatPhrase() != null && !ctx.writeFormatPhrase().isEmpty()) {
				final ValueStmt formatValueStmt = createValueStmt(ctx.writeFormatPhrase().get(0).identifier(),
						ctx.writeFormatPhrase().get(0).literal());
				result.setFormatPhrase(formatValueStmt);
			}

			// IBM ILE: terminal phrase
			if (ctx.writeTerminalPhrase() != null) {
				final ValueStmt terminalValueStmt = createValueStmt(ctx.writeTerminalPhrase().identifier(),
						ctx.writeTerminalPhrase().literal());
				result.setTerminalPhrase(terminalValueStmt);
			}

			// IBM ILE: starting phrase
			if (ctx.writeStartingPhrase() != null) {
				if (ctx.writeStartingPhrase().identifier() != null) {
					final ValueStmt startingValueStmt = createValueStmt(ctx.writeStartingPhrase().identifier());
					result.setStartingPhrase(startingValueStmt);
				} else if (ctx.writeStartingPhrase().integerLiteral() != null) {
					final ValueStmt startingValueStmt = createValueStmt(ctx.writeStartingPhrase().integerLiteral());
					result.setStartingPhrase(startingValueStmt);
				}
			}

			// IBM ILE: indicators phrase
			if (ctx.writeIndicatorsPhrase() != null) {
				final Call indicatorsCall = createCall(ctx.writeIndicatorsPhrase().identifier());
				result.setIndicatorsPhrase(indicatorsCall);
			}
