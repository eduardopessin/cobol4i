...
public abstract class DataDescriptionEntryContainerImpl extends CobolDivisionElementImpl
	...
	public DataDescriptionEntryExecSql addDataDescriptionEntryExecSql(final DataDescriptionEntryExecSqlContext ctx) {
		...
		if (result == null) {
			...
			final java.util.List<org.antlr.v4.runtime.tree.TerminalNode> allTokens = new java.util.ArrayList<>(ctx.EXECSQLLINE());
			if (ctx.EXECSQLEND() != null) {
				for (final org.antlr.v4.runtime.tree.TerminalNode endNode : ctx.EXECSQLEND()) {
					allTokens.add(endNode);
				}
			}
			allTokens.sort((a, b) -> Integer.compare(a.getSymbol().getTokenIndex(), b.getSymbol().getTokenIndex()));
			final String execSqlText = TagUtils.getUntaggedText(allTokens, CobolPreprocessor.EXEC_SQL_TAG,
	...
	public DataDescriptionEntryGroup addDataDescriptionEntryGroup(final DataDescriptionEntryFormat1Context ctx) {
		...
		if (result == null) {
			 ...
			 * like clause
			 */
			final List<DataLikeClauseContext> dataLikeClauseContexts = ctx.dataLikeClause();

			if (!dataLikeClauseContexts.isEmpty()) {
				final DataLikeClauseContext dataLikeClauseContext = dataLikeClauseContexts.get(0);
				result.addLikeClause(dataLikeClauseContext);
			}

			/*
			 * indicator clause
			 */
			final List<DataIndicatorClauseContext> dataIndicatorClauseContexts = ctx.dataIndicatorClause();

			if (!dataIndicatorClauseContexts.isEmpty()) {
				final DataIndicatorClauseContext dataIndicatorClauseContext = dataIndicatorClauseContexts.get(0);
				result.addIndicatorClause(dataIndicatorClauseContext);
			}

			/*
			 * format clause
			 */
			final List<DataFormatClauseContext> dataFormatClauseContexts = ctx.dataFormatClause();

			if (!dataFormatClauseContexts.isEmpty()) {
				final DataFormatClauseContext dataFormatClauseContext = dataFormatClauseContexts.get(0);
				result.addFormatClause(dataFormatClauseContext);
			}
