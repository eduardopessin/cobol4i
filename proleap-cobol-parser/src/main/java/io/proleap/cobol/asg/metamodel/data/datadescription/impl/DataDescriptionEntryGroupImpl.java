...
public class DataDescriptionEntryGroupImpl extends DataDescriptionEntryImpl implements DataDescriptionEntryGroup {
	...
	@Override
	public FormatClause addFormatClause(final DataFormatClauseContext ctx) {
		FormatClause result = (FormatClause) getASGElement(ctx);

		if (result == null) {
			result = new FormatClauseImpl(programUnit, ctx);

			final FormatClause.FormatType formatType;

			if (ctx.DATE() != null) {
				formatType = FormatClause.FormatType.DATE;
			} else if (ctx.TIME() != null) {
				formatType = FormatClause.FormatType.TIME;
			} else if (ctx.TIMESTAMP() != null) {
				formatType = FormatClause.FormatType.TIMESTAMP;
			} else {
				formatType = null;
			}

			result.setFormatType(formatType);
	...
	@Override
	public IndicatorClause addIndicatorClause(final DataIndicatorClauseContext ctx) {
		IndicatorClause result = (IndicatorClause) getASGElement(ctx);

		if (result == null) {
			result = new IndicatorClauseImpl(programUnit, ctx);

			if (ctx.integerLiteral() != null) {
				result.setIndicatorNumber(Integer.parseInt(ctx.integerLiteral().getText()));
			}

			indicatorClause = result;
			registerASGElement(result);
		}
	...
	@Override
	public LikeClause addLikeClause(final DataLikeClauseContext ctx) {
		LikeClause result = (LikeClause) getASGElement(ctx);

		if (result == null) {
			result = new LikeClauseImpl(programUnit, ctx);

			final Call likeCall = createCall(ctx.qualifiedDataName());
			result.setLikeCall(likeCall);

			if (ctx.integerLiteral() != null) {
				result.setLengthModifier(Integer.parseInt(ctx.integerLiteral().getText()));
			}
	...
	@Override
	public FormatClause getFormatClause() {
		return formatClause;
	}
	...
	@Override
	public IndicatorClause getIndicatorClause() {
		return indicatorClause;
	}
	...
	@Override
	public LikeClause getLikeClause() {
		return likeClause;
	}
