...
public class DisplayStatementImpl extends StatementImpl implements DisplayStatement {
	...
	public At addAt(final DisplayAtContext ctx) {
		...
		if (result == null) {
			...
			// displayAt may have identifiers (LINE/COLUMN) or a single identifier/literal
			if (!ctx.identifier().isEmpty()) {
				final ValueStmt atValueStmt = createValueStmt(ctx.identifier().get(0));
				result.setAtValueStmt(atValueStmt);
			} else if (ctx.literal() != null) {
				final ValueStmt atValueStmt = createValueStmt(ctx.literal());
				result.setAtValueStmt(atValueStmt);
			}
