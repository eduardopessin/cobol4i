...
startRule
   : (compilerOptions | copyDdsStatement | copyStatement | execCicsStatement | execSqlStatement | execSqlImsStatement | replaceOffStatement | replaceArea | ejectStatement | skipStatement | titleStatement | charDataLine | NEWLINE)* EOF
   ;
...
// copy DDS statement (IBM i Format 2 COPY)

copyDdsStatement
   : COPY copyDdsSource (OF | IN) copyDdsFileName (NEWLINE* copyDdsOption)* NEWLINE* DOT
   ;

copyDdsSource
   : (DD_PREFIX | DDR_PREFIX | DDS_PREFIX | DDSR_PREFIX) (IDENTIFIER | ALL_FORMATS)
   ;
...
copyDdsOption
   : WITH? (I_FIELDS | O_FIELDS | I_O_FIELDS)
   | INDICATOR NUMERICLITERAL?
   | NULL_MAP
   | PREFIX LPARENCHAR (cobolWord | literal) (COMMACHAR NUMERICLITERAL)? RPARENCHAR
   | ALIAS
   | SUBSTITUTE
   | VLR
   | SUPPRESS
...
charDataKeyword
   ...
   | YEARWINDOW | YW
   | ZWB
   | OUTPUT | NOMONOPRC | VARCHAR | DATETIME | NODATE | NOTIME
   | INDICATOR | NULL_MAP | SUBSTITUTE | VLR | WITH | PREFIX
...
// IBM ILE compiler options
OUTPUT : O U T P U T;
NOMONOPRC : N O M O N O P R C;
...
// DDS copy tokens
DD_PREFIX : D D MINUSCHAR;
DDR_PREFIX : D D R MINUSCHAR;
DDS_PREFIX : D D S MINUSCHAR;
DDSR_PREFIX : D D S R MINUSCHAR;
ALL_FORMATS : A L L MINUSCHAR F O R M A T S;
INDICATOR : I N D I C A T O R;
I_FIELDS : I MINUSCHAR F I E L D S;
O_FIELDS : O MINUSCHAR F I E L D S;
I_O_FIELDS : I MINUSCHAR O MINUSCHAR F I E L D S;
